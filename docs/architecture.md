...
- That chokepoint only covers *configuration* failures. A key that exists but is expired, rate-limited, or paired with a wrong model passes `is_available()` and fails mid-request, so `orchestrator._complete_with_runtime_fallback()` catches a live `ProviderUnavailableError` from any non-Ollama provider and retries on Ollama, setting the same `fell_back` flag. Ollama's own failures propagate to the error path — there is nothing behind it to retry.
