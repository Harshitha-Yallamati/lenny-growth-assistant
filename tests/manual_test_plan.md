...
| 9b | Claude Agent SDK present in the image | `docker compose exec backend python -c "from app.llm.claude_agent_provider import sdk_available; print(sdk_available())"` and `docker compose exec backend which claude` | Prints `True` and a path — confirms both halves of the SDK's runtime contract (pip package + Claude Code CLI) are installed |
| 9c | Agent SDK degradation | Run the same import in a local venv without `claude-agent-sdk`, then check which provider the registry picks | `sdk_available()` is `False` and the Anthropic slot resolves to `AnthropicProvider` (native Messages API) — the app must still start |
